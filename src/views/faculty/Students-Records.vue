          ...
          <table class="min-w-max border-collapse text-sm">
            <thead>
              ...
              <tr class="bg-white border-b border-gray-200">
                ...
                <th class="px-4 py-3 text-center text-sm font-bold text-green-600 bg-green-50 w-28">
                  Status
                </th>
              </tr>
                  ...
                  <template v-if="hasFinalWA(student.id)">
                    <select
                      ...
                      @change="
                        handleStatusChange(
                          student.id,
                          ($event.target as HTMLSelectElement).value as 'passed' | 'failed' | null,
                        )
                      "
                    ...
                    <div v-if="updatingStatus === student.id" class="mt-1">
                      <div
                        class="animate-spin inline-block w-3 h-3 border-2 border-current border-t-transparent text-green-600 rounded-full"
                      ></div>
                    </div>
                  </template>
                  <span v-else class="text-gray-300 text-xs">—</span>
